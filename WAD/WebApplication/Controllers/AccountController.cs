...
{
    /// <summary>
    /// Defines the account controller class.
    /// </summary>
    public class AccountController : Controller
    {
        ...
        /// <param name="signInManager">Application sign in manager.</param>
        ...
        /// <summary>
        /// Action result to the login page.
        /// </summary>
        /// <param name="returnUrl"></param>
        /// <returns>The lohin page.</returns>
        [AllowAnonymous]
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            var user = db.Users.Where(m => m.Email == model.Email).FirstOrDefault();
        ...
        [HttpGet]
        [Route("ConfirmEmail?userId={userId&code={code}}")]
        [AllowAnonymous]
        public async Task<ActionResult> ConfirmEmail(string userId, string code)
        {
            ...
            if (user != null)
            {
                ...
                {
                    return RedirectToAction("InvalidCode", "Account", new { Email = user.Email });
                }
            }
            else
            {
                return RedirectToAction("InvalidUser", "Account", new { Email = "" });
            }
        ...
        public void LoginTime(string userName)
        {
            ...
            using (var db = new ApplicationDbContext())
            {
                ...
                var model = new LoginHistory(DateTime.UtcNow, ip, user.Id);                
                db.LoginListory.Add(model);
                db.SaveChanges();
            }
        }
        ...
        public void LogOutTime(string userId)
        {
            using (var db = new ApplicationDbContext())
            {
                var model = db.LoginListory.Where(u => u.UserId == userId).OrderByDescending(u => u.LoginHistoryId).First();
        ...
        public void CreateCustomUser(string userId)
        {
            using (var db = new ApplicationDbContext())
            {
                var user = db.Users.Where(m => m.Id == userId).FirstOrDefault();
                var model = new CustomUser(userId);
                db.CustomUsers.Add(model);
                db.SaveChanges();
            }
        }
